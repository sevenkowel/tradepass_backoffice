      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
        <Card className="w-full max-w-md">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <p className="text-gray-600">正在跳转至 Console...</p>
          </CardContent>
        </Card>
      </div>
    );
        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-6 space-y-5">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">验证码已发送至 {otpTarget}</p>
            </div>
            ...
            <button onClick={sendOTP} disabled={otpSending} className="w-full text-sm text-primary hover:underline">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
      <Card className="w-full max-w-md">
        <CardContent className="p-6 space-y-5">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900">注册 TradePass</h1>
            <p className="text-sm text-gray-500 mt-1">创建 Console 管理账户</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {config.agreements.map((agr) => (
                  <label key={agr.id} className="flex items-start gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-600">
                      我已阅读并同意<span className="text-primary hover:underline cursor-pointer">《{agr.title}》</span>
                      {agr.required && <span className="text-red-500 ml-0.5">*</span>}
                    </span>
                  </label>
                ))}
                {fieldErrors[`agreement_${config.agreements[0]?.id}`] && (
                  <p className="text-xs text-red-500">请阅读并同意所有必填协议</p>
                )}
              </div>
            )}
            ...
            <Button type="submit" disabled={loading} className="w-full bg-primary hover:bg-primary-dark text-white">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>注册 <ArrowRight className="w-4 h-4 ml-1" /></>}
            </Button>
          </form>

          <p className="text-center text-sm text-gray-500">
            已有账号？<Link href="/auth/login" className="text-primary hover:underline">立即登录</Link>
          </p>
        </CardContent>
      </Card>
    </div>
